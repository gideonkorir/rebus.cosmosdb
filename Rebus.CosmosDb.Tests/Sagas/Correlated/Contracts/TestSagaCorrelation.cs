                              ...
                              IHandleMessages<GuidMessage>
        {
            ...
            protected override void CorrelateMessages(ICorrelationConfig<MySagaData> config)
            {
                if (!_useSagaDataPropertyNames)
                {
                    ...
                    config.CorrelateHeader<IntMessage>("saga-id", p => p.Id);
                    config.CorrelateHeader<GuidMessage>("saga-id", p => p.Id); 
                    config.CorrelateHeader<StringMessage>("saga-id", p => p.Id);
                }
                else
                {
                    config.Correlate<Initiate>(m => Guid.NewGuid(), nameof(MySagaData.Id)); 
                    config.CorrelateHeader<IntMessage>("saga-id", nameof(MySagaData.Id));
                    config.CorrelateHeader<GuidMessage>("saga-id", nameof(MySagaData.Id));
                    config.CorrelateHeader<StringMessage>("saga-id", nameof(MySagaData.Id));
                }
            }
            ...
            public async Task Handle(Initiate message)
            {
                ...
                {
                    ["saga-id"] = Data.Id.ToString("D", System.Globalization.CultureInfo.InvariantCulture)
                };

                await Task.WhenAll(
                    _bus.SendLocal(new IntMessage { AnInt = Data.IntValue }, header),
                    _bus.SendLocal(new StringMessage { AString = Data.StringValue }, header),
                    _bus.SendLocal(new GuidMessage { AGuid = Data.GuidValue }, header)
                    );
            }
